...
**Not yet done:** rendering (CopyBits + QuickDraw primitives into the shim's
screen buffer, then out to a browser canvas or SDL), mouse/keyboard events,
menus, PICT decoding for per-adventure floor tiles (`new.interior`), sound.
